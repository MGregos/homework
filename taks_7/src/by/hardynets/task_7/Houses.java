...
// Мы народ не особо богатый, поэтому ограничимся участком в 100 соток
        ...
        if (e <= 0 || e > 100) {
            System.out.println("Сторона участка 'e' слишком большая, или введено неверное значение. Уходим.");
            System.exit(0);
        ...
        int f = in.nextInt();
        if (f <= 0 || f > 100) {
            System.out.println("Сторона участка 'f' слишком большая, или введено неверное значение. Уходим.");
            System.exit(0);
            }
            if (f > e) {
                temp = e;
        ...
        // Теперь нам нужно убрать из участка наш дом. Создадим 2 новых прямоугольника
            ...
            int c = in.nextInt();
            if (c <= 0 || c > 100) {
                System.out.println("Сторона 1 дома 2 слишком большая, или введено неверное значение. Уходим.");
                System.exit(0);
